...
- status: done
- owner: Codex
- branch: feature/codex-tag-exclude-filter
- priority: medium
...
- summary: Tag exclude filter implemented; typecheck/build passed
...
- [x] タグタブの各タグ行に「絞り込む」「除外」ボタンが表示される
- [x] 除外ボタンを押すと該当タグを持つ投稿が一覧から消える
- [x] `bookmarked` タグを除外するといいね投稿のみが表示される
- [x] include と exclude を同じタグに同時設定できない（一方をセットすると他方が解除される）
- [x] 「すべての絞り込みを解除」で exclude も解除される
- [x] フィルターモーダルのタグタブバッジが exclude 適用中でも表示される
- [x] セッション復元で exclude フィルターが引き継がれる
...
- `2026-04-20 Codex`: Implemented excludeTagFilter across archive queries, viewer loading, filter state, modal UI, session persistence, and local tag updates.

## Codex Result

- Added `excludeTagFilter` to list and bulk tag filter inputs, with archive query handling that subtracts excluded tag post IDs from the include/author/date match set.
- Added viewer state, load request propagation, session persistence/restore, tag rename/merge handling, and clear-all support for exclude filters.
- Updated the unified filter modal so each tag row exposes Include and Exclude controls, with tag tab active state and selected exclude summaries.
- Updated local tag add behavior so adding the currently excluded tag removes the post from the visible page immediately.

## Verification

- `cmd /c npm run typecheck` passed.
- `cmd /c npm run build` passed.
- Shared Profile manual verification passed with CDP Chrome on port 9223.
- Verified tag modal rows show Include / Exclude controls for `ブックマーク`.
- Verified excluding `ブックマーク` changed the viewer count from `表示中 50件 / 全 16,690件` to `表示中 50件 / 全 13,477件`, showed one active filter, and the first visible page had 0 bookmark-tagged posts.
- Verified the tag modal badge remained active and the `ブックマーク` exclude button had `aria-pressed="true"`.
- Verified switching the same tag to include changed the viewer count to `表示中 50件 / 全 3,213件`; the first 20 visible posts all had `ブックマーク` in IndexedDB.
- Verified clear-all returned the viewer to `表示中 50件 / 全 16,690件` with no active filter badge.
- Verified session restore by applying exclude, reloading the Viewer, and confirming the restored count was `表示中 50件 / 全 13,477件` with the `ブックマーク` exclude button still pressed; cleared the filter afterward.
