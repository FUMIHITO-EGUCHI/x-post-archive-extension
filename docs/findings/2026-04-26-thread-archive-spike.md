...
- Chrome 147、`.shared-cdp-profile`（dev X アカウントログイン済）
...
|---|---|---|---|---|
| S1 head | (redacted) | 200 | 925 | ✅ 3 件 |
| S1 mid | (redacted) | 200 | 386 | ✅ 3 件（中央 ID 起点でも root から） |
| S2 | (redacted) | 200 | ~400 | ✅ 2 件 |
| S3 (Thread Compose 5) | (redacted) | 200 | 1001 | ✅ 5 件 + 後続続編 3 件 |

### 取得構造（Sample 3 抜粋）

```
root <post-id-A> (user <OP_user_id>)
  └ <post-id-B> (user 同)
    └ <post-id-C>
      └ <post-id-D>
        └ <post-id-E> (5/5)
