...
- \`wxt.config.ts\` の \`manifest.name\` → **Offline X Archive** に揃える（現状 \`X Post Archive Extension\`、CWS 提出前に更新が必要）
- \`wxt.config.ts\` の \`manifest.description\` → 上 § Short description (EN) と揃える（現状 \`Save X posts and review them later in a simple archive viewer.\`、提出前に更新）
- \`PRIVACY.md\` 冒頭の "X Post Archive Extension" 表記も **Offline X Archive** に揃える（リスティング名と Privacy Policy 内表記が一致しないと CWS レビュー時に指摘される可能性がある）

> ⚠ \`wxt.config.ts\` と \`PRIVACY.md\` の name 更新は別 Issue で扱う（提出直前に行う）。本ドキュメントが先行する。
