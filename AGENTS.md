...
- このリポジトリでは、agent はユーザーから明示的な指示がない限り `git push` を実行しないこと
...
- agent が Git 作業を扱うときは、コミット案・コミットメッセージ案・ブランチ作業・マージ作業・バージョン更新作業の説明に `docs(likes-import): ...` や `feat(likes-import): ...` のような形式で補助ラベルを付けて、対象タスクが識別できるようにすること
