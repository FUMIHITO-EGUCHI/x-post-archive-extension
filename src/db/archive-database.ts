...
export class ArchiveDatabase extends Dexie {
  ...
  constructor() {
    ...
    // v18: Compound indexes for keyset pagination on sortable fields.
    // Why: offset(N) on IndexedDB advances the cursor N times → O(N).
    // Compound `[field+x_post_id]` lets `where("[field+x_post_id]").below([value, xPostId])` resolve in O(log N).
    this.version(18).stores({
      posts:
        "&x_post_id, saved_at, posted_at, reply_count, repost_count, like_count, display_name, quoted_post_id, in_reply_to_post_id, thread_root_id, x_username, [x_username+saved_at], [saved_at+x_post_id], [posted_at+x_post_id], [reply_count+x_post_id], [repost_count+x_post_id], [like_count+x_post_id]",
      media: "&media_id, x_post_id, [x_post_id+position], storage_status, saved_at, media_type",
      tags: "&tag_id, &normalized_name, system_key, display_name, created_at",
      tag_redirects:
        "&tag_redirect_id, &source_normalized_name, source_display_name, target_tag_id, created_at",
